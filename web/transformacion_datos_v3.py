    ...
    if run_process:
        ...
        if show_graphs:
            ...
            with st.spinner("Generando gráficos..."):
                ...
                # Nuevo: Matriz de correlación y heatmap
                st.markdown("#### Matriz de Correlación y Heatmap")
                corr_matrix = df_sismicidad.corr()
                ...
                # Nuevo: Matriz de dispersión para variables numéricas
                st.markdown("#### Matriz de Dispersión (Scatter Matrix)")
                num_df = df_sismicidad.select_dtypes(include=np.number)
                fig_scatter_matrix = px.scatter_matrix(num_df, title="Scatter Matrix de Variables Numéricas")
                st.plotly_chart(fig_scatter_matrix, use_container_width=True)
                
                # Nuevo: Boxplots de variables clave (ejemplo: 'Local Magnitude' y 'Mo')
                st.markdown("#### Boxplots")
                fig_box1 = px.box(df_sismicidad, y="Local Magnitude", title="Distribución de Local Magnitude")
                st.plotly_chart(fig_box1, use_container_width=True)
                fig_box2 = px.box(df_sismicidad, y="Mo", title="Distribución de Mo")
        ...
        with st.spinner("Asignando disparos a eventos sísmicos..."):
            ...
            if hours_after_option != 'Manual':
                ...
                console_relacion += f"⏲ Delta max: {delta_max_calc} hrs. | Delta min: {delta_min_calc} hrs. | Delta promedio: {int(np.mean(deltas).total_seconds()/3600)} hrs.\n"
            ...
            for i in range(len(df_tronaduras) - 1):
                ...
                if hours_after_option == 'Mínimo':
                    ...
                    fecha_fin = fecha_inicio + pd.Timedelta(hours=delta_max_calc)
                elif hours_after_option == 'Promedio':
                    fecha_fin = fecha_inicio + pd.Timedelta(hours=delta_mean_calc)
                elif (hours_after_option == 'Manual') and (manual_hours_after > 0):
                    fecha_fin = fecha_inicio + pd.Timedelta(hours=manual_hours_after)
                else:
                    fecha_fin = df_tronaduras.loc[i + 1, 'Fecha']
