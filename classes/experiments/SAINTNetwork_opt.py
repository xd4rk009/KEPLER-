# SAINTNetwork_opt.py
# Implementación "raw" de SAINT para regresión tabular, compatible con BaseTrainer_opt
...
class SAINTNetwork_opt(BaseTrainer_opt):
    """
    SAINT (Self-Attention and Intersample Attention Transformer) para regresión tabular.
    Hereda de BaseTrainer_opt.
    """
    ...
    def forward(self, x_cont, x_cat=None):
        ...
        # Self-Attention (intra-sample)
        x = self.transformer(x)
        # Intersample Attention (SAINT):
        # Para simplificar, aplicamos pooling global (media) como aproximación
        x = x.mean(dim=1)
    ...
    def train_model(self, X_train, y_train, X_val, y_val, num_epochs=100, lr=1e-3):
        ...
        for epoch in range(num_epochs):
            ...
            if self.show_info == "True" and (epoch % 100 == 0 or epoch == num_epochs - 1):
                self._print(f"Epoch {epoch+1}/{num_epochs} - Loss: {loss.item():.4f}", level="progress", net_name="SAINT")
        for module in [self.cat_embeds, self.cont_proj, self.transformer, self.regressor]:
    ...
    def save_model(self, model_name="SAINT", ext="pt"):
